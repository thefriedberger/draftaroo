...
const WatchlistStar = ({ player }: { player: Player }) => {
   ...
   useEffect(() => {
      const watchedPlayers = watchlist?.find((watchedPlayer: Player) => {
         return watchedPlayer.id === player.id;
      });
      if (watchedPlayers !== undefined) {
         setIsWatched(watchedPlayers.id === player.id);
      } else {
         setIsWatched(false);
      }
   }, [watchlist]);
   return (
      <>
         <span className="w-full" onClick={() => handleUpdateWatchlist(player)}>
            {isWatched ? (
